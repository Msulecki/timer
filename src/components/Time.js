...
class Time extends Component {
        ...
        if (this.props.timerActive) {
            ...
            if ((this.state.lastScramble !== this.props.scramble)) {
                if ((this.state.lastScramble.length > 0) && document.querySelector('.time__clock').textContent !== '00:00.00' && this.state.resultDate !== this.props.resultDate) {
                    this.setState({
                        results: [...this.state.results, [document.querySelector('.time__clock').textContent, this.state.lastScramble, this.props.resultDate]],
                        resultsUpdated: true,
    ...
    render() {
        return (
            <div className="time" onClick={this.props.started.bind(this, new Date().getTime())}>
                <div className="time__scramble">{!this.props.timerActive && (this.state.results.length > 0) && this.state.results[this.state.results.length - 1][1].join(' ')}</div> {/* If timer isn't active, show last element */}
